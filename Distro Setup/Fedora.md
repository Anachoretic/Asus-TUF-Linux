...
4. `akmod-nvidia`: Automatically builds the kernel module on new kernel updates.
5. `xorg-x11-drv-nvidia-cuda`: Adds CUDA support if you're doing compute work.
...
3. `asusctl`: Manages performance profiles, LED effects, and fan control.
4. `supergfxctl`: Lets you switch between GPU modes.
5. `rog-control-center`: GUI tool for controlling Asus features.
...
{% hint style="info" %}
Some hotkeys are BIOS-level and can’t be changed. To check if one is remappable, try assigning it in your shortcut settings. If it registers, you’re good.
{% endhint %}
